...
if [ -n "$TOKEN" ]; then
  ...
  if [ "$HTTP_CODE" = "404" ]; then
    curl -s -X POST "http://127.0.0.1:${PORT:-8090}/api/collections" \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer $TOKEN" \
      -d '{"name":"boxes","type":"base","schema":[{"name":"deviceId","type":"text"},{"name":"deviceName","type":"text"},{"name":"status","type":"text"},{"name":"command","type":"text"},{"name":"ipAddress","type":"text"},{"name":"lastSeen","type":"text"},{"name":"androidVersion","type":"text"}],"listRule":"","viewRule":"","createRule":"","updateRule":"","deleteRule":""}'
  fi
fi
